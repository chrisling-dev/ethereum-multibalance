...
import { encodeOwnerAndTokens } from "../utils/encodeOwnerAndTokens"
...
describe("MultiBalanceV2", async function () {
  ...
  console.log("Deploying MultiBalanceV2 contract")
  const multibalance = await viem.deployContract("MultiBalanceV2")
  ...
  describe("getBalancesForOwner", async function () {
    it("should return a valid balance", async function () {
      const start = performance.now()
      const balances = await client.readContract({
        abi: MultiBalanceAbi,
        address: multibalance.address,
        functionName: "getBalancesForOwner",
        args: [encodeOwnerAndTokens(HOLDER, [WETH])],
      })

      assert.equal(balances.length, 1)
      assert.equal(balances[0], HOLDER_WETH_BALANCE)
      console.log(`MultiBalance done in ${performance.now() - start}ms`)
    })
